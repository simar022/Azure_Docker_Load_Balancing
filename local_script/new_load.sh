...
force_cleanup() {
    local name=$1
    local pid=$(sudo docker inspect --format '{{.State.Pid}}' $name 2>/dev/null)
    if [ ! -z "$pid" ] && [ "$pid" != "0" ]; then
...
sudo systemctl reload apparmor 2>/dev/null
...
http {
    ...
    upstream static_sites {
        server website1:80 max_fails=3 fail_timeout=10s;
        server website2:80 max_fails=3 fail_timeout=10s;
        server website3:80 max_fails=3 fail_timeout=10s;
    }
    server {
       listen 80;
       location / {
...
while True:
    ...
    if rps > RPS_THRESHOLD and current_scale < MAX_REPLICAS:
        ...
        os.system(f"sudo docker-compose up -d --scale website1={MAX_REPLICAS} --scale website2={MAX_REPLICAS} --scale website3={MAX_REPLICAS}")
        ...
        if now - last_scale_time > COOLDOWN_PERIOD:
            ...
            os.system(f"sudo docker-compose up -d --scale website1={MIN_REPLICAS} --scale website2={MIN_REPLICAS} --scale website3={MIN_REPLICAS}")
            current_scale = MIN_REPLICAS
        else:
...
sudo docker stats --no-stream --format "table {{.Name}}\t{{.MemUsage}}\t{{.NetIO}}\t{{.CPUPerc}}"; \
